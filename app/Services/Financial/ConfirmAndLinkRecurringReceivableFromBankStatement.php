...
class ConfirmAndLinkRecurringReceivableFromBankStatement
{
    public function __construct(private readonly ConfirmRecurringFinancialExpectation $confirm, private readonly LinkAccountReceivableFromBankStatement $link) {}
    ...
    public function execute(Wallet $wallet, BankAccount $bankAccount, JournalEntry $entry, RecurringFinancialExpectation $expectation, CarbonInterface $period, CarbonInterface $dueDate, ?string $notes = null): RecurringFinancialOccurrence
    {
        return DB::transaction(function () use ($wallet, $bankAccount, $entry, $expectation, $period, $dueDate, $notes) {
            ...
            if ($expectation->wallet_id !== $wallet->id || $expectation->type !== 'receivable') {
                throw ValidationException::withMessages(['expectation' => 'Recorrência a receber inválida para esta carteira.']);
            }
            ...
            $this->link->execute($wallet, $bankAccount, $entry, $occurrence->accountReceivable()->firstOrFail());

            return $occurrence->fresh(['accountReceivable.receiptJournalEntry', 'accountReceivable.provisionJournalEntry']);
        });
    }
}
