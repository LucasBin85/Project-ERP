...
export function useAccountsReceivableIndex(filters: AccountsReceivableFilters) {
    const form = reactive({
    ...
    watch(() => form.start_date, () => {
        if (!form.start_date) form.start_date = startOfMonthLocal();
        if (form.start_date > form.end_date) form.end_date = form.start_date;
    });

    watch(() => form.end_date, () => {
        if (!form.end_date) form.end_date = form.start_date;
        if (form.end_date < form.start_date) form.start_date = form.end_date;
    });

    useAutoFilters(form, 'accounts-receivable.index');
