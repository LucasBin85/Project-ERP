...
<template>
    <AppLayout title="Fechamento Mensal">
        <ReportPage title="Fechamento Mensal" :subtitle="`${closing.period.label} · ${wallet.name}`">
            ...
            <ReportSection>
                <template #header><div><h2 class="font-bold text-white">Recorrências previstas não resolvidas</h2><p class="text-sm text-gray-400">Revisão gerencial informativa; estas previsões não impedem o fechamento formal.</p></div></template>
                <div v-if="!closing.recurring_review.has_pending" class="p-5 text-sm text-green-300">Nenhuma recorrência prevista permanece sem revisão neste mês.</div>
                <div v-else class="grid gap-4 p-4 lg:grid-cols-2">
                    <div v-for="(group, type) in { payables: closing.recurring_review.payables, receivables: closing.recurring_review.receivables }" :key="type" class="rounded-lg border border-amber-700/50 bg-amber-950/20 p-4">
                        <div class="flex items-start justify-between gap-3"><div><h3 class="font-semibold text-white">{{ type === 'payables' ? 'A pagar' : 'A receber' }}</h3><p class="text-sm text-gray-300">{{ group.count }} competência(s) · {{ formatCurrency(group.estimated_amount_cents) }} estimados</p><p v-if="group.unestimated_count" class="text-xs text-amber-300">{{ group.unestimated_count }} sem estimativa</p></div><Link :href="group.url" class="whitespace-nowrap text-sm text-indigo-300">{{ type === 'payables' ? 'Revisar contas a pagar' : 'Revisar contas a receber' }}</Link></div>
                        <ul v-if="group.items.length" class="mt-3 divide-y divide-amber-900/40 text-sm"><li v-for="item in group.items.slice(0, 5)" :key="`${item.expectation_id}-${item.period_date}`" class="flex items-center justify-between gap-3 py-2"><span><b class="block text-gray-200">{{ item.description }}</b><small class="text-gray-400">{{ item.counterparty?.name ?? 'Sem contraparte' }} · {{ item.due_date }}</small></span><span class="whitespace-nowrap" :class="item.has_estimated_amount ? 'text-gray-200' : 'text-amber-300'">{{ item.has_estimated_amount ? formatCurrency(item.expected_amount_cents) : 'Sem estimativa' }}</span></li></ul>
