        ...
        <ReportPage title="Fluxo de Caixa" :subtitle="wallet.name">
            <ReportSection>
                ...
                <div class="grid grid-cols-1 gap-4 p-6 md:grid-cols-2 xl:grid-cols-5">
                    <div>
                        ...
                        <input
                            v-model="cashFlow.form.start_date"
                            type="date"
                    ...
                    <div>
                        ...
                        <input
                            v-model="cashFlow.form.end_date"
                            type="date"
                            class="w-full rounded-lg border border-gray-700 bg-black px-3 py-2 text-white [color-scheme:dark]"
                        />
                    </div>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <ReportSummaryCard
                    ...
                    :value="formatCurrency(summary.realized_closing_balance_cents)"
                    :tone="summary.realized_closing_balance_cents >= 0 ? 'green' : 'red'"
                />
                <ReportSummaryCard
                    label="Saldo projetado"
                    :value="formatCurrency(summary.projected_closing_balance_cents)"
                    :tone="summary.projected_closing_balance_cents >= 0 ? 'green' : 'red'"
                />
                <ReportSummaryCard
                    label="Resultado projetado"
                    :value="formatCurrency(summary.projected_net_cents)"
                    :tone="summary.projected_net_cents >= 0 ? 'green' : 'red'"
                />
            </div>
            ...
            <div
                v-if="summary.unestimated_projected_items_count > 0"
                class="rounded-lg border border-amber-800/60 bg-amber-950/30 px-4 py-3 text-sm text-amber-200"
            >
                Há {{ summary.unestimated_projected_items_count }} recorrência(s) projetada(s) sem estimativa de valor. Os saldos projetados não
                incluem esses itens.
            </div>
                    ...
                    <div class="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
                        <div>
                            ...
                            <p class="text-sm text-gray-400">
                                Realizado vem do extrato bancário; projetado vem de contas a pagar, contas a receber, faturas de cartão e recorrências
                                esperadas.
                            </p>
                        </div>
                ...
                <ReportTable :empty="items.length === 0" empty-message="Nenhum item de fluxo encontrado para o período." :empty-colspan="9">
                    <template #head>
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-bold text-gray-400 uppercase">Cliente/Fornecedor</th>
                            ...
                            <th class="px-4 py-3 text-right text-xs font-bold text-gray-400 uppercase">Saldo realizado</th>
                            <th class="px-4 py-3 text-right text-xs font-bold text-gray-400 uppercase">Saldo projetado</th>
                    ...
                    <tr v-for="item in items" :key="item.id" class="hover:bg-gray-800/50">
                        <td class="px-4 py-3 text-sm whitespace-nowrap text-gray-300">{{ formatDate(item.date) }}</td>
                        ...
                        <td class="px-4 py-3 text-right text-sm font-semibold whitespace-nowrap text-green-300">
                            ...
                            <template v-else>{{
                                item.amount_cents > 0 ? `${item.amount_mode === 'variable' ? '~ ' : ''}${formatCurrency(item.amount_cents)}` : '-'
                            }}</template>
                        </td>
                        <td class="px-4 py-3 text-right text-sm font-semibold whitespace-nowrap text-red-300">
                            ...
                            <template v-else>{{
                                item.amount_cents < 0
                                    ? `${item.amount_mode === 'variable' ? '~ ' : ''}${formatCurrency(Math.abs(item.amount_cents))}`
                                    : '-'
                            }}</template>
                        </td>
                        <td class="px-4 py-3 text-right text-sm whitespace-nowrap text-gray-300">
                            {{ formatCurrency(item.running_realized_balance_cents) }}
                        </td>
                        <td
                            class="px-4 py-3 text-right text-sm font-semibold whitespace-nowrap"
                            :class="item.running_projected_balance_cents >= 0 ? 'text-green-300' : 'text-red-300'"
                        >
                            {{ formatCurrency(item.running_projected_balance_cents) }}
                            <span
                                v-if="item.running_projected_balance_complete === false"
                                class="ml-1 text-xs font-normal text-amber-300"
                                title="Existem projeções anteriores sem estimativa de valor."
                                >Parcial</span
                            >
                        </td>
