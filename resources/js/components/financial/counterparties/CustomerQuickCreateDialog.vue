...
const props = withDefaults(defineProps<{ show: boolean; suggestedName?: string; controlAccounts?: any[]; revenueAccounts?: any[]; existingNames?: string[] }>(), {
    suggestedName: '', controlAccounts: () => [], revenueAccounts: () => [], existingNames: () => [],
});
    ...
    <Teleport to="body">
        <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" @click.self="emit('close')">
            <form class="w-full max-w-lg space-y-4 rounded-xl border border-gray-700 bg-gray-950 p-6 shadow-xl" @submit.prevent="submit">
                ...
                <div><label class="mb-1 block text-sm text-gray-300">Nome</label><input v-model="form.name" autofocus class="w-full rounded bg-black p-3 text-white"><p v-if="duplicateName" class="text-sm text-red-400">Já existe um cliente com este nome.</p><p v-else class="text-sm text-red-400">{{ errors.name?.[0] }}</p></div>
                ...
                <div class="flex justify-end gap-3"><button type="button" class="rounded border border-gray-600 px-4 py-2 text-gray-300" @click="emit('close')">Cancelar</button><button :disabled="!canSubmit" class="rounded bg-indigo-600 px-4 py-2 text-white disabled:cursor-not-allowed disabled:opacity-50">Cadastrar cliente</button></div>
            </form>
        </div>
    </Teleport>
</template>
